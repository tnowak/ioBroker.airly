...
class Airly extends utils.Adapter {
  ...
  async onReady() {
    ...
    if (!apikey) {
      this.log.error(
        "Airly API key is not configured. Open the adapter settings and enter it.",
      );
      return;
    }
  ...
  async writeMeasurements(payload) {
    ...
    if (!states) {
      this.log.warn(
        'Airly response contained no "current" measurements (point out of coverage?).',
      );
      return;
    }
  ...
  logQuota(headers = {}) {
    ...
    if (remaining !== undefined && limit !== undefined) {
      this.log.debug(
        `Airly daily quota: ${remaining}/${limit} calls remaining`,
      );
    }
  }
  ...
  handleError(err) {
    if (err.response) {
      ...
      if (status === 401) {
        this.log.error(
          "Airly API rejected the key (401). Check the apikey in the settings.",
        );
      } else if (status === 404) {
        this.log.warn(
        ...
        this.log.warn(
          "Airly API rate limit reached (429). Increase the poll interval.",
        );
      } else {
        this.log.error(
          `Airly API error ${status}: ${JSON.stringify(err.response.data)}`,
        );
      }
    } else {
